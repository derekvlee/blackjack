...
public class BJStrategyRulesTest {
	...
	private static PlayerHandInfo ph1;
	...
	public void testBJStrategyRules() {
		...
		assertEquals(null, BJStrategyRules.callRules(new PlayerHandInfo(mockedPlayerHand)));
		
		when(mockedUpCardI.getValue()).thenReturn(9);
		when(mockedPlayerHand.getCardsValue()).thenReturn(15);
		when(mockedPlayerHand.getDealersUpCard()).thenReturn(mockedUpCardI);
		assertEquals(BJStrategy.HIT, BJStrategyRules.callRules(new PlayerHandInfo(mockedPlayerHand)));
		
		when(mockedUpCardI.getValue()).thenReturn(3);
		when(mockedPlayerHand.getCardsValue()).thenReturn(15);
		when(mockedPlayerHand.getDealersUpCard()).thenReturn(mockedUpCardI);
		assertEquals(BJStrategy.STAND, BJStrategyRules.callRules(new PlayerHandInfo(mockedPlayerHand)));
		...
		when(mockedPlayerHand.getDealersUpCard()).thenReturn(mockedUpCardI);
		assertEquals(BJStrategy.DOUBLE, BJStrategyRules.callRules(new PlayerHandInfo(mockedPlayerHand)));
	}
